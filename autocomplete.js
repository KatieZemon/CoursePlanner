    ...
    */
    ...
    /**
    * Attaches the autocomplete object to a form element. Sets
    * onkeypress event on the form element.
    * 
    * @param string formElement Name of form element to attach to
    * @param array  data        Array of strings of which to use as the autocomplete data
    */
    function AutoComplete_Create (id, data, lowerdata)
    {
		...
		//__AutoComplete[id]['element'].setParent(document.getElementById(id), __AutoComplete[id]['element']);
		document.getElementById('courseTable').appendChild(__AutoComplete[id]['element']);
        ...
        __AutoComplete[id]['element'].onkeypress = function(e) {
																	...
																	//if ( contains(data, (__AutoComplete[id]['element'].value + keychar)) ) {
																	if (keynum != 8 && lowerdata.indexOf((__AutoComplete[id]['element'].value + keychar).toLowerCase())>=0) {
																		document.getElementById('newclassbutton').disabled = false;
																	} else if (keynum == 8 && lowerdata.indexOf((__AutoComplete[id]['element'].value.substring(0, __AutoComplete[id]['element'].value.length-1)).toLowerCase())>=0) {
																		document.getElementById('newclassbutton').disabled = false;
																	}else{
																		document.getElementById('newclassbutton').disabled = true;
																	} 
																	if (e.keyCode == 13 || isOpera) return false; 
																	__AutoComplete[id]['element'].style.textAlign = "left";
																}
        __AutoComplete[id]['element'].ondblclick = function() {AutoComplete_ShowDropdown(this.getAttribute('id'));}
        __AutoComplete[id]['element'].onclick    = function(e) {if (__AutoComplete[id]['element'].value == "< Insert Course Name >") {document.getElementById(id).focus(); document.getElementById(id).select();} if (!e) e = window.event; e.cancelBubble = true; e.returnValue = false;}
    ...
    */
    function AutoComplete_ShowDropdown(id)
    {
        ...
        if (value.length == 0) {
          return AutoComplete_HideDropdown(id);
        }
    ...
    */
    function AutoComplete_SetValue(id)
    {
        ...
        __AutoComplete[id]['element'].value = __AutoComplete[id]['dropdown'].childNodes[__AutoComplete[id]['highlighted']].innerHTML;
		  if (list.indexOf(__AutoComplete[id]['element'].value)>=0) {
		    document.getElementById('newclassbutton').disabled = false;
	  	}
    }
